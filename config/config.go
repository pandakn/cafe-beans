...
func LoadConfig(path string) IConfig {
	...
	return &config{
		app: &app{
			...
			// create callback anonymous func for convert APP_PORT from type string to int
			port: func() int {
			...
			readTimeout: func() time.Duration {
				...
				// duration return nanoseconds
				// Convert to seconds by multiplying 10^9
		...
		db: &db{
			...
			port: func() int {
				...
				if err != nil {
					log.Fatalf("load db post failed: %v", err)
				}
				return p
			}(),
		...
		jwt: &jwt{
			...
			accessExpiresAt: func() int {
				...
				if err != nil {
					log.Fatalf("load jwt access token expires failed: %v", err)
				}
				return t
			}(),
			refreshExpiresAt: func() int {
				...
				if err != nil {
					log.Fatalf("load jwt refresh token expires failed: %v", err)
				}
				return t
			}(),
		},
...
type IAppConfig interface {
	Url() string // host:port for listen app
	Name() string
	Version() string
...
func (c *db) MaxOpenConns() int { return c.maxConnections }
